...
EnemyRapier::EnemyRapier(int x, int y) :Enemy(x, y) {
	...
	EnemyRapierTextures = App->textures->Load("HERE GOES THE PATH, LUCHO CHECK THE PATH ");
	...
	void EnemyRapier::Move() {
		...
		position.x += (speed+1)*0.02*(playerPosition.x - position.x); //MOVEMENT
		position.y += speed*0.02*(playerPosition.y - position.y);
	}
	/*void EnemyRapier::OnCollision(Collider *c1, Collider *c2) {

	}*/
	void EnemyRapier::OnCollision(Collider *c1) {
