...
EnemyBile::EnemyBile(int x, int y) :Enemy(x, y) {
	...
	EnemyBileTextures = App->textures->Load("HERE GOES THE PATH, LUCHO CHECK THE PATH ");
...
void EnemyBile::Move() {
	...
	position.x += (speed*0.3) * 0.02*(playerPosition.x - position.x);  //MOVEMENT
	position.y += (speed*0.3) * 0.02*(playerPosition.y - position.y);
}

void EnemyBile::OnCollision(Collider *c1) {
